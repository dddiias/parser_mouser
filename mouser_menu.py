...
"""
Простой интерактивный режим (меню) поверх существующего mouser_cli.py.
Не меняет код CLI — только переиспользует его функции.

Возможности:
- Ввод одного или нескольких парт-номеров (через пробел или запятую)
- Показ результата в виде таблицы
- Сохранение результатов в CSV/JSON
- (опционально) Сохранение "сырых" ответов API в папку
...
# Импортируем готовые функции из твоего mouser_cli.py
...
def _split_pns(s: str) -> List[str]:
    # Разбиваем по запятой/пробелам; фильтруем пустые токены
...
def _lookup_one(pn: str, api_key: str, retries: int = 3, timeout: int = 20,
    ...
    # Валидация токена, чтобы не слать мусор в API
    if not _is_valid_pn(pn):
        print(f"  ⚠️  Пропускаю некорректный ввод: {pn!r}")
        return None

    # 1) пробуем как Mouser PN
    try:
    ...
    # 2) если пусто — пробуем keyword (подходит для manufacturer PN)
    if not part:
        try:
    ...
    # Покажем краткую сводку
    print("  ✓ Готово.")
    return row


def main():
    # Проверяем ключ (берётся из .env)
    try:
    ...
    while True:
        ...
        if choice == "0":
            ...
            # Сбрасываем предыдущий буфер результатов
            results = []
            for pn in pns:
                print(f"  → Ищу: {pn} ...")
